...
const AdminIncomeWorks = () => {
  const { associateId, clientName } = useParams();
  ...
  const load = async () => {
    ...
    try {
      const res = await axiosInstance.get(
        `/payouts/admin/associates/${associateId}/works`,
  ...
  useEffect(() => { load(); }, [associateId, clientName]);
    ...
    <DashboardLayout activeMenu="Income">
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <button onClick={() => navigate(`/admin/income/${associateId}/clients`)}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <StatCard icon={CheckCircle2} title="Total Payout Paid"    value={fmt(totals.payout)}  color="green" />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={9} className="py-12 text-center text-gray-400">No work income records found.</td></tr>
                ) : pagedWorks.map((w) => (
                  <tr key={String(w.invoiceId)} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        onClick={() => navigate(`/admin/income/payout/${w.invoiceId}`)}
...
export default AdminIncomeWorks;