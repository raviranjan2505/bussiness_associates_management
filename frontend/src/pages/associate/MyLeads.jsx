...
const MyLeads = () => {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      const haystack = `${lead.leadId || ""} ${lead.clientDetails?.clientName || ""} ${lead.clientDetails?.mobileNumber || ""} ${lead.clientDetails?.email || ""} ${lead.leadStatus || ""}`.toLowerCase();
      ...
      <div className="p-6 space-y-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Track the status of your submitted leads.</p>
          </div>
        ...
        <div className="rounded-lg border border-gray-100 bg-white p-4">
          <input
            ...
            placeholder="Search by lead id, client, or status"
            value={search}
                  ...
                  filteredLeads.map((lead) => (
                    <tr key={lead._id} className="border-t hover:bg-gray-50">
                      ...
                      <td className="p-3 text-gray-600">
                        ...
                        <div className="text-xs text-gray-500">{lead.division?.name || ""}</div>
                      </td>
